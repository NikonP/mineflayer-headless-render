...
function bedrockRoot() {
  return path.join(getBedrockPath(), 'resource_pack')
}
function modelDir() {
  return path.join(bedrockRoot(), 'models')
}
function entityDir() {
  return path.join(bedrockRoot(), 'entity')
}
function textureDir() {
  return path.join(bedrockRoot(), 'textures')
}
...
function entityDef(base) {
  ...
  const description =
    def &&
    def['minecraft:client_entity'] &&
    def['minecraft:client_entity'].description
...
function pickTexture(textures) {
  const keys = Object.keys(textures).filter(
    k => typeof textures[k] === 'string'
  )
...
function buildEntityModel(name) {
  ...
  const texPath =
    ...
    textureOverride[base] ||
    pickTexture(desc.textures || {})
  if (!texPath) return null
  const layers = []
  for (const key of [primary, ...(extraGeometryKeys[name] || [])]) {
    const geometry =
      desc.geometry[key] && buildGeoIndex().get(desc.geometry[key])
  ...
  for (const { geometry } of layers) {
    ...
    for (const bone of geometry.bones) {
      ...
      for (const cube of bone.cubes) {
        ...
        const cubeMat = cube.rotation
          ? matFromEuler(eulerFromDegrees(cube.rotation))
          : null
        for (const { corners, u0, v0, u1, v1 } of Object.values(elemFaces)) {
          ...
          for (const pos of corners) {
            ...
            let x =
              cube.origin[0] +
              pos[0] * cube.size[0] +
              (pos[0] ? inflate : -inflate)
            let y =
              cube.origin[1] +
              pos[1] * cube.size[1] +
              (pos[1] ? inflate : -inflate)
            let z =
              cube.origin[2] +
              pos[2] * cube.size[2] +
