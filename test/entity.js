...
const names = process.argv.slice(2).length ? process.argv.slice(2) : ['zombie', 'creeper', 'cow']
...
const floorAtlas = (() => {
  ...
  for (let i = 0; i < 4; i++) { data[i * 4] = 200; data[i * 4 + 1] = 200; data[i * 4 + 2] = 200; data[i * 4 + 3] = 255 }
...
for (const name of names) {
  ...
  if (!model) { console.error(name, ': no model'); continue }
  ...
  const mesh = composeEntityMesh(model, { position: { x: 0, y: 0, z: 0 }, yaw: 0 })
  renderMesh(frame, vp, mesh, model.texture, { backfaceCull: false })
  const png = new PNG({ width: frame.width, height: frame.height })
  png.data = frame.data
  fs.writeFileSync(path.join(outDir, 'unit_' + name + '.png'), PNG.sync.write(png))
