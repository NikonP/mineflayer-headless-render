...
async function main () {
  ...
  bot.once('spawn', async () => {
    ...
    console.log('')
    console.log('step  cold x  cold total/world | warm x  tick: wall/meshed/calls | warm total/world | cache')
    console.log('-'.repeat(104))

    const measure = () => {
      const timing = {}
      const t0 = performance.now()
      const frame = pov.render({ width: 640, height: 360, timeOfDay: 6000, timing })
      const total = performance.now() - t0
      frameToPng(frame)
      return { total, timing }
    }
    const tp = async (x) => { bot.chat(`/tp ${bot.username} ${x} ${p.y} ${p.z}`); await wait(200) }
