...
function stats (arr) {
  ...
  const at = (p) => s[Math.min(s.length - 1, Math.max(0, Math.ceil(p * s.length) - 1))]
  return { mean: sum / s.length, med: at(0.5), p95: at(0.95), min: s[0], max: s[s.length - 1] }
}

const f1 = (n) => n.toFixed(1)
const mb = (n) => (n / 1024 / 1024).toFixed(1) + 'MB'
const gcNow = () => { if (global.gc) global.gc() }
...
async function buildScene (bot) {
  ...
  const mobs = ['zombie', 'cow', 'sheep', 'pig', 'chicken', 'creeper', 'skeleton', 'spider']
  for (let i = 0; i < mobs.length; i++) {
    await cmd(`/summon minecraft:${mobs[i]} ${p.x - 4 + i * 1.2} ${p.y} ${p.z + 4.5} {NoAI:1}`)
  }
  const items = ['diamond', 'apple', 'stick', 'redstone', 'coal', 'egg']
  for (let i = 0; i < items.length; i++) {
    await cmd(`/summon minecraft:item ${p.x - 4 + i * 1.2} ${p.y + 0.1} ${p.z + 7.5} {Item:{id:"minecraft:${items[i]}",count:1}}`)
  }
  await cmd(`/fill ${p.x - 6} ${p.y} ${p.z + 9} ${p.x + 6} ${p.y + 3} ${p.z + 9} minecraft:stone_bricks`)
...
async function main () {
  ...
  const configs = parseConfigs(args.configs || process.env.CONFIGS || '640x360@6,1280x720@6,1280x720@12')
  ...
  bot.once('spawn', async () => {
    ...
    const p = bot.entity.position.floored()
    console.log(`spawn at ${p} | frames=${frames} warmup=${warmup} scene=${scene} cache=${useCache}`)
    ...
    renderFrame(bot, { width: 320, height: 180, viewDistance: 2, timeOfDay: 6000, meshCache })
    gcNow()
    const rssStart = process.memoryUsage().rss

    console.log('')
    console.log('config              total(ms)          fps | world light terr ent | encode | rss     | cpu/frame')
    console.log('-'.repeat(112))

    for (const c of configs) {
      for (let i = 0; i < warmup; i++) renderFrame(bot, { width: c.w, height: c.h, viewDistance: c.view, timeOfDay: 6000, meshCache })
      gcNow()
      ...
      for (let i = 0; i < frames; i++) {
        ...
        const frame = renderFrame(bot, { width: c.w, height: c.h, viewDistance: c.view, timeOfDay: 6000, timing, meshCache })
    ...
    console.log('')
    console.log('notes: world = meshing (cached or redone every frame), light = per-face light bake,')
    console.log('       terr = terrain raster, ent = entities, encode = PNG encode. cpu% is of one core.')

    if (scene) { bot.chat('/kill @e[type=!minecraft:player]'); await wait(400) }
    clearTimeout(timer)
    bot.quit()
